...
def run_it():
    ...
    if not os.path.exists(conv_dir):
        ...
        if (os.path.isfile(f'{conv_dir}/{conv_prefix}_0.00.{conv_ext}') and  
            ...
            print(f'Convergence data already exist in {conv_dir}.')
    ...
    rotation_files = [os.path.join(dirpath, f) for (dirpath, dirnames, filenames) in os.walk(Utils.get_parameter('rotation_dir')) for f in filenames]
    topology_files = [os.path.join(dirpath, f) for (dirpath, dirnames, filenames) in os.walk(Utils.get_parameter('topology_dir')) for f in filenames]
    ...
    for age in reversed(range(start_time, end_time+1, time_step)):
        ...
        column_list = ['trench_lon', 'trench_lat', 'conv_rate_cm_yr', 'conv_angle_deg', 'trench_abs_rate_cm_yr', 'trench_abs_angle_deg',
            'arc_len_deg', 'trench_norm_deg', 'subducting_pid', 'trench_pid', 'dist_nearest_edge_deg', 'dist_from_start_deg',
            'conv_ortho_cm_yr', 'conv_paral_cm_yr', 'trench_abs_ortho_cm_yr', 'trench_abs_paral_cm_yr', 'subducting_abs_rate_cm_yr',
            'subducting_abs_angle_deg', 'subducting_abs_ortho_cm_yr', 'subducting_abs_paral_cm_yr']
        ...
        for grid in Utils.get_parameter('grid_files'):
            ...
            if grid_name == 'seafloor_age_ma':
                ...
                arc_length_m = 2*math.pi*6371000*trench_data.arc_len_deg/360

                # Calculate Subduction Volume (in m^3 per year)
                subduction_volume_m3y = trench_data.conv_ortho_cm_yr/100 * thickness * arc_length_m
                ...
                trench_data['subduction_volume_km3_yr'] = subduction_volume_km3y
                
        trench_data.to_csv(conv_dir + conv_prefix + f'_{age}.00.csv', index=False, float_format='%.2f', na_rep='nan')
